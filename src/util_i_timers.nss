    ...
    if (!nIterations || nRemaining)
    {
        ...
        if (!nIterations || nRemaining)
        {
            ...
            Notice("Scheduling next iteration for timer " + sTimerID + ":" + sMsg +
                   "\n    Delay: " + FloatToString(fInterval, 0, 2) + "\n    Remaining: " +
                   (nIterations ? (IntToString(nRemaining)) : "INFINITE") + "/" + sIterations);
...
int GetIsTimerValid(int nTimerID)
{
    ...
    CreateTimersTable();
    sqlquery q = SqlPrepareQueryModule("SELECT 1 FROM timers WHERE timer_id = @timer_id;");
    SqlBindInt(q, "@timer_id", nTimerID);
...
{
    CreateTimersTable();
    sqlquery q = SqlPrepareQueryModule("UPDATE timers SET running = 1, run_id = run_id + 1 " +
...
void ResetTimer(int nTimerID)
{
    ...
    if (SqlStep(q))
    {
        Notice("ResetTimer(" + IntToString(nTimerID) + ") successful: " + IntToString(SqlGetInt(q, 0)) +
...
{
    CreateTimersTable();
    sqlquery q = SqlPrepareQueryModule("DELETE FROM timers WHERE timer_id = @timer_id RETURNING 1;");
    SqlBindInt(q, "@timer_id", nTimerID);
    if (SqlStep(q))
    {
...
{
    CreateTimersTable();
    sqlquery q = SqlPrepareQueryModule("SELECT iterations FROM timers WHERE timer_id = @timer_id;");
    SqlBindInt(q, "@timer_id", nTimerID);
    return SqlStep(q) ? !SqlGetInt(q, 0) : FALSE;
}
...
{
    CreateTimersTable();
    sqlquery q = SqlPrepareQueryModule("SELECT remaining FROM timers WHERE timer_id = @timer_id;");
    SqlBindInt(q, "@timer_id", nTimerID);
...
{
    CreateTimersTable();
    sqlquery q = SqlPrepareQueryModule("UPDATE timers SET remaining = @remaining " +
